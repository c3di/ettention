...
import etomo.ui.swing.EttentionsetupDisplay;
...
public final class ApplicationManager extends BaseManager implements
  ...
  /**
   * Updates ettentionsetup.com.  Created ettentionsetup.com if it is not there, and required is
   * true.
   * @param axisID
   * @param display
   * @param required
   * @return
   */
  public EttentionsetupParam updateEttentionSetupCom(final AxisID axisID,
      final EttentionsetupDisplay display, final boolean required, final boolean doValidation) {
	  ParallelPanel parallelPanel = getMainPanel().getParallelPanel(axisID);
   /* ParallelPanel parallelPanel = getMainPanel().getParallelPanel(axisID);
    if (parallelPanel == null) {
      if (required) {
        System.out
            .println("Error: eTomo is attempting to run Ettention without parallel processing.");
      }
      return null;
    }*/
    File ettentionSetupComFile = FileType.ETTENTIONSETUP_COMSCRIPT.getFile(this, axisID);
    EttentionsetupParam param = null;
    if (!ettentionSetupComFile.exists()) {
      if (!required) {
        return null;
      }
      MakecomfileParam makecomFileParam = new MakecomfileParam(this, axisID,
          FileType.ETTENTIONSETUP_COMSCRIPT);
      makecomfile(axisID, makecomFileParam);
      comScriptMgr.loadEttentionsetup(axisID);
    }
    param = comScriptMgr.getEttentionsetupParam(axisID);
    if (!display.getParameters(param, doValidation)) {
      return null;
    }
    if (!parallelPanel.getParameters(param, doValidation)) {
    ...
    comScriptMgr.saveEttentionsetup(param, axisID);
    return param;
  }
  
  public void msgEttentionsetupSucceeded(final AxisID axisID) {
	    ((TomogramGenerationExpert) getUIExpert(DialogType.TOMOGRAM_GENERATION, axisID))
	        .msgEttentionsetupSucceeded();
	  }
  ...
  public void ettentionsetup(final AxisID axisID,
	      final ProcessResultDisplay processResultDisplay, ProcessSeries processSeries,
	      final DialogType dialogType, final ProcessingMethod processingMethod,
	      final EttentionsetupDisplay display) {
	    sendMsgProcessStarting(processResultDisplay);
	    if (updateTiltCom(
	        ((TomogramGenerationExpert) getUIExpert(dialogType, axisID)).getTiltDisplay(),
	        axisID, true) == null) {
	    ...
	    EttentionsetupParam param = updateEttentionSetupCom(axisID, display, true, true);
	    if (param == null) {
	      sendMsg(ProcessResult.FAILED_TO_START, processResultDisplay);
	      return;
	    }
	    ...
	    String threadName;
	    try {
	      threadName = processMgr.ettentionsetup(axisID, processResultDisplay, processSeries,
	          param, processingMethod);
	    }
	    catch (SystemProcessException e) {
	      e.printStackTrace();
	      String[] message = new String[2];
	      message[0] = "Can not execute " + ProcessName.ETTENTIONSETUP;
	      message[1] = e.getMessage();
	      uiHarness.openMessageDialog(this, message, "Unable to execute command", axisID);
	      sendMsg(ProcessResult.FAILED_TO_START, processResultDisplay);
	      return;
	    }
	    processSeries.setNextProcess(ProcessName.ETTENTIONSETUP.toString(),
	        ProcessName.ETTENTIONSETUP, processingMethod);
	    processSeries.setLastProcess(TomogramGenerationExpert.ETTENTION_DONE);
	    setThreadName(threadName, axisID);
	    mainPanel.startProgressBar("Running " + ProcessName.ETTENTIONSETUP, axisID,
	        ProcessName.ETTENTIONSETUP);
	    return;
	  }
  ...
  public boolean useEttention(final ProcessResultDisplay processResultDisplay,
	      final File useFile, final String runButtonLabel, final AxisID axisID,
	      final DialogType dialogType) {
	    FileType outputFileType = FileType.TILT_OUTPUT;
	    return useImageFile(processResultDisplay, FileType.ETTENTION_OUTPUT_TEMPLATE, useFile,
	        outputFileType, runButtonLabel, axisID, dialogType);
  }
