...
public class TomogramGenerationDialog extends ProcessDialog implements ContextMenu {
  ...
  private final RadioButton rbEttention = new RadioButton("Ettention", bgMethod);

  private final TiltPanel tiltPanel;
  private final TomogramGenerationExpert expert;
  private final SirtPanel sirtPanel;
  private final EttentionPanel ettentionPanel;
  ...
  private TomogramGenerationDialog(ApplicationManager appMgr,
    ...
    ettentionPanel = EttentionPanel.getInstance(appMgr, axisID, dialogType, btnAdvanced, this);
    tiltPanel.addFieldObserver(sirtPanel);
    tiltPanel.addFieldObserver(ettentionPanel);
    sirtPanel.addResumeObserver(tiltPanel);
    sirtPanel.addFieldObserver(ettentionPanel);
    ettentionPanel.addResumeObserver(tiltPanel);
    ettentionPanel.addFieldObserver(sirtPanel);
  }
  ...
  private void createPanel() {
    ...
    rootPanel.add(ettentionPanel.getRoot());
    // method panel
    ...
    pnlMethod.add(Box.createHorizontalGlue());
    pnlMethod.add(rbEttention.getComponent());
    pnlMethod.add(Box.createHorizontalGlue());
    // buttons
  ...
  private void addListeners() {
    ...
    rbEttention.addActionListener(listener);
  }
  ...
  void setParameters(ConstMetaData metaData) {
    ...
    ettentionPanel.setParameters(metaData);
    methodChanged();
  }
  ...
  void setParameters(final EttentionsetupParam param) {
	    ettentionPanel.setParameters(param);
  }
  ...
  final void setParameters(ReconScreenState screenState) {
    ...
    ettentionPanel.setParameters(screenState);
  }
  ...
  private void updateAdvanced() {
    ...
    ettentionPanel.updateAdvanced(isAdvanced());
  ...
  public void popUpContextMenu(MouseEvent mouseEvent) {
    ...
    boolean ettentionInUse = rbEttention.isSelected();
    int numManpages = 2;
    int numLogs = 1;
    if (sirtInUse) {
      numManpages = 3;
    ...
    else if (ettentionInUse) {
    	logFileLabel[i] = "Final Ettention";
        logFile[i++] = "tilt" + axisID.getExtension() + "_ettention-finish" + ".log";
        manPageLabel[i] = "Ettentionsetup";
        manPage[i++] = "ettentionsetup.html";
  ...
  EttentionsetupDisplay getEttentionsetupDisplay() {
	    return ettentionPanel;
  }
  ...
  private void action(final ActionEvent event) {
    ...
    if (actionCommand.equals(rbBackProjection.getActionCommand())
        ...
        || actionCommand.equals(rbEttention.getActionCommand())) {
      methodChanged();
    }
  }
