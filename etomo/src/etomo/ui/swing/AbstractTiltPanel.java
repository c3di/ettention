...
abstract class AbstractTiltPanel implements Expandable, TrialTiltParent,
  ...
  void setVisible(final boolean advanced) {
    ...
    boolean ettention = isEttention();
    cbParallelProcess.setVisible(backProjection);
    ltfLogDensityScaleOffset.setVisible(advanced && !ettention);
    ltfLogDensityScaleFactor.setVisible(advanced && !ettention);
    ltfLinearDensityScaleOffset.setVisible(advanced && !ettention);
    ltfLinearDensityScaleFactor.setVisible(advanced && !ettention);
    ...
    ltfTiltAngleOffset.setVisible(advanced && !ettention);
    ...
    if(ettention)
    {
    	cbUseLocalAlignment.setSelected(false);
    	cbUseZFactors.setSelected(false);
    	cbUseLocalAlignment.setEnabled(false);
    	cbUseZFactors.setEnabled(false);
    	ltfZShift.setVisible(false);
    	cbUseZFactors.setToolTipText("Ettention currently does not support Z factors.");
    	cbUseLocalAlignment.setToolTipText("Ettention currently does not support local alignments.");
    	ctfLog.setLabel("Take logarithm of densities with maximum density value as na offset");
    	ctfLog.setTextFieldVisible(false);
    	ctfLog.setToolTipText("Take logarithm of densities with maximum density value as na offset.");
    }
    else
    {
    	ctfLog.setLabel("Take logarithm of densities with offset: ");
    	ctfLog.setTextFieldVisible(true);
    	ctfLog.setToolTipText("This parameter allows one to generate a reconstruction using the "
    	...
    	cbUseZFactors.setToolTipText("Use the file containing factors for adjusting the backprojection position "
    	...
    	cbUseLocalAlignment.setToolTipText("Select this checkbox to use local alignments.  You must have "
  ...
  void updateDisplay() {
    ...
    cbUseGpu.setEnabled((((gpusAvailable || localGpuAvailable) && (cbParallelProcess
        .isSelected() || isSirt() || isEttention())) || (localGpuAvailable
        && !cbParallelProcess.isSelected() && !isSirt() && !isEttention()))
