...
namespace ettention
{
    ...
    void CommandLineParameterSource::parse()
    {
        ...
        store(boost::program_options::command_line_parser(arguments).options(options).style(options::command_line_style::default_style | options::command_line_style::allow_long_disguise ^ options::command_line_style::allow_guessing).run(), vm);
        notify(vm);
    }
    ...
    CommandLineParameterSource::LogLevelValues CommandLineParameterSource::LogLevel() const
    {
        return logLevel;
    }

    void CommandLineParameterSource::LogLevel(const ParameterSource::LogLevelValues level)
    {
        logLevel = level;
    }
