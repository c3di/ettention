...
namespace ettention
{
    ...
    cl_context CLContextCreator::CreateContext(const HardwareParameterSet* hardwareParameterSet)
    {
        ...
        if(hardwareParameterSet->ExplicitDeviceId() != -1)
        {
            if(hardwareParameterSet->ExplicitDeviceId() < (int)availableDevices.size())
            {
                auto platformDevice = availableDevices[hardwareParameterSet->ExplicitDeviceId()];
                context = this->CreateContextFromDevice(platformDevice);
            }
            else
            {
                LOGGER_IMP("Explicit device id " << hardwareParameterSet->ExplicitDeviceId() << " is invalid.");
            }
        }
        if(!context)
        {
            auto devices = this->GetPreferredDevices(hardwareParameterSet->PreferedOpenCLPlatform(), hardwareParameterSet->PreferedOpenCLDevice());
    ...
    cl_context CLContextCreator::CreateContextFromDevice(const PlatformDevice& platformDevice) const
    {
        ...
        LOGGER("OpenCL device: " << info.name << ".");
        return context;
    }
