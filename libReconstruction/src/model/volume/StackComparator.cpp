...
#include "framework/RootMeanSquareError.h"
...
namespace ettention
{
    ...
    {
        if(firstVolume->Properties().GetVolumeResolution() != secondVolume->Properties().GetVolumeResolution())
            throw std::runtime_error("illegal comparison of volumes with different size");

        auto dataOne = firstVolume->ConvertToFloat();
        auto dataTwo = secondVolume->ConvertToFloat();

        return RootMeanSquareError::computeRMSError(dataOne.get(), dataTwo.get(), firstVolume->Properties().GetVolumeVoxelCount());
    }
