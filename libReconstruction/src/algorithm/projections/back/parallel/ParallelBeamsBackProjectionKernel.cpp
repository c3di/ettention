...
namespace ettention
{
    ...
    std::vector<VoxelSample> ParallelBeamsBackProjectionKernel::generateSamplingPatternForPerVoxelSampling()
    {
        ...
        auto voxelSize = volume->getObjectOnCPU()->Properties().GetVoxelSize();
        float maxVoxelSize = fmax(fmax(voxelSize.x, voxelSize.y), voxelSize.z);
        Vec3f h = Normalize(geometricSetup->getScannerGeometry().getHorizontalPitch()) * maxVoxelSize;
        Vec3f v = Normalize(geometricSetup->getScannerGeometry().getVerticalPitch()) * maxVoxelSize;
        ...
        for(float y = 0.0f; y < (float)getSamplesPerDirection(); y += 1.0f)
        {
            ...
            for(float x = 0.0f; x < (float)getSamplesPerDirection(); x += 1.0f)
            {
                ...
                auto tLimits = GeometricAlgorithms::ClipRayAgainstBoundingBox(Ray::FromPointAndDirection(samplePos, geometricSetup->getScannerGeometry().getRayDirection()), aabb);
