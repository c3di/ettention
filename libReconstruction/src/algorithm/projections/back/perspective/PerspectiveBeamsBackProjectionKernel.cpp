...
namespace ettention
{
    ...
    std::string PerspectiveBeamsBackProjectionKernel::getKernelDefines(Framework* framework)
    {
        ...
        {
            defines = defines + (boost::format("-D VOLUME_MINIMUM_INTENSITY %04d") % *paramSet->getVolumeMinimumIntensity()).str();
        }
        if(paramSet->getVolumeMaximumIntensity() != boost::none)
        {
            defines = defines + (boost::format("-D VOLUME_MAXIMUM_INTENSITY %04d") % *paramSet->getVolumeMaximumIntensity()).str();
        }
        return defines;
    }
