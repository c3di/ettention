...
TEST_F(Matrix4x4Test, RotationFromYawPitchRoll)
{
    ...
    {
        Matrix4x4 m = Matrix4x4::RotationFromYawPitchRoll(90.0f, 0.0f, 0.0f);
    ...
    {
        Matrix4x4 m = Matrix4x4::RotationFromYawPitchRoll(0.0f, 90.0f, 0.0f);
    ...
    {
        Matrix4x4 m = Matrix4x4::RotationFromYawPitchRoll(0.0f, 0.0f, 90.0f);
...
TEST_F(Matrix4x4Test, ProjectionTest)
{
    ...
    Matrix4x4 perspectiveProjection = Matrix4x4::PerspectiveProjection(eyePos, projectionPlaneBasePos, projectionPlaneDx, projectionPlaneDy, projectionResolution);
    Matrix4x4 parallelProjection = Matrix4x4::ParallelProjection(eyePos, projectionPlaneBasePos, projectionPlaneDx, projectionPlaneDy);
    ...
    for(unsigned int x = 0; x < projectionResolution.x; ++x)
    {
        for(unsigned int y = 0; y < projectionResolution.y; ++y)
        {
            ...
            ASSERT_NEAR((float)x, transformedPerspective.x, EPS);
            ASSERT_NEAR((float)y, transformedPerspective.y, EPS);
            ...
            ASSERT_NEAR((eyePos - projectionPlaneBasePos).LengthF(), transformedParallel.z, EPS);
