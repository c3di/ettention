...
namespace ettention
{
    namespace etomo
    {
        ...
        {
            Vec3f outputVolumeDimension = Vec3f(0.0f, 0.0f, 0.0f);
            ...
            while(getline(tiltFile, line))
            {
                if(line[0] != '#' && line[0] != '$')
                {
                    ...
                    {
                        outputVolumeDimension.z = boost::lexical_cast<float>(paramValue);
                    }
                    ...
                    {
                        storage.insertOrRewriteScalarParameter<bool>("input.logarithmize", "bool", true);
                    }
                }
            }
            ...
            if(!skipProjectionsList.empty())
            {
                storage.insertOrRewriteScalarParameter<std::string>("input.skipProjections", "string", skipProjectionsList);
            }
        ...
        {
            storage.insertOrRewriteScalarParameter<std::string>("output.format", "string", "mrc");
        ...
        void EtomoParameterSource::binVolumeDimensions(Vec3f& dimensions, unsigned int binning)
        {
            if(binning != 0)
            {
