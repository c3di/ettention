    ...
    namespace stem
    {
        TF_ART_ParameterSet::TF_ART_ParameterSet(const ParameterSource* parameterSource)
        {
            ...
            if(convergentBeam && useAutoFocus)
                markerFileName = parameterSource->getPathParameter("input.markerFilename");
        ...
        float TF_ART_ParameterSet::getBeamOpeningHalfAngle() const
        {
        ...
        TF_ART_ParameterSet::BackProjectionMode TF_ART_ParameterSet::parseBackProjectionMethod(std::string methodString)
        {
            ...
            if(methodString.compare("stem.adjoint") == 0)
                return ADJOINT;
            throw Exception("parse error in ConvergentBeamsParameterSet::BackProjectionMethod: illegal method " + methodString + " legal values are stem.regularization and stem.adjoint" );
        }
