...
boost::posix_time::ptime lwpt_ofile;
boost::posix_time::ptime lwpt_ifile;
...
#define sizeDeclaration         "const long int %s_size;\n"
#define sizeMacroDefinition     "#define %s_size %d\n"
#define typeName                "const char"
...
const char *inputFileName;  /* Current output file name */
...
static void processFile(const char *fileName, long int from, long int to)
{
    ...
    std::time_t lwt_ofile = boost::filesystem::last_write_time(fileName);
    lwpt_ifile = boost::posix_time::from_time_t(lwt_ofile);

    if (boost::filesystem::exists(outputFileName) && (lwpt_ofile > lwpt_ifile ) ) {
        exit(0);
    }
    else
        fprintf(stderr, "compiling and escaping %s to %s\n", fileName, outputFileName);
    ...
    output("%s %s%s[%d] = {\n", typeName, symbolName, "_Array", to - from + 2);
    if (headerFile) {
        header("extern std::string %s_SourceCode;\n", symbolName);
    }
    ...
    output("std::string %s_SourceCode( %s_Array );", symbolName, symbolName );
...
static void openOutputFile(const char *fileName)
{
    ...
    {
        std::time_t lwt_ofile = boost::filesystem::last_write_time(fileName);
        lwpt_ofile = from_time_t(lwt_ofile);
    }
