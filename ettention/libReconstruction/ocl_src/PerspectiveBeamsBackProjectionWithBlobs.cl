...
{ 
    ...
    const uint id = gid.z * resY * resX + gid.y * resX + gid.x;
    ...
    const float4 blobCenter = (float4)((volumeGeometry->bBoxMin + (float4)((float)gid.x + 0.5f, (float)gid.y + 0.5f, (float)gid.z + 0.5f, 0.0f) * volumeGeometry->voxelSize).xyz, 1.0f);
