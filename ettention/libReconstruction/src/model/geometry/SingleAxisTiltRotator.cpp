...
namespace ettention
{
    SingleAxisTiltRotator::SingleAxisTiltRotator()
        ...
        , wasBaseScannerGeometrySet(false)
    {
    }

    SingleAxisTiltRotator::~SingleAxisTiltRotator()
    {
        if( wasBaseScannerGeometrySet && (!this->baseScannerGeometry) )
            LOGGER((boost::format("EXTRA MEMORY RELEASE: SingleAxisTiltRotator DTOR trying to delete NULLPTR baseScannerGeometry\n")).str());
    ...
    {
        if( this->baseScannerGeometry == baseScannerGeometry )
            return;

        delete this->baseScannerGeometry;
        this->baseScannerGeometry = baseScannerGeometry;
        wasBaseScannerGeometrySet = true;
    }
