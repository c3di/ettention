...
namespace ettention
{
    ...
    std::string CLAbstractionLayer::getDeviceInfo<std::string>(cl_device_id deviceId, cl_device_info info)
    {
        ...
        if( charCount  >= 1 )
            result.resize(charCount-1);    // clGetDeviceInfo char string ends with \0 and we don't need it
        ...
        , imageSupport(false)
    {
        ...
        checkImageSupport(hardwareParameterSet->isImageSupportDisabled());
        if(hardwareParameterSet->shouldPrintDeviceInfo())
        {
    ...
    CLAbstractionLayer::~CLAbstractionLayer()
    {
        ...
        {
            LOGGER_ERROR("Still " << aliveCLMemObjects.size() << " unreturned CLMemObject(s) and " << aliveCLSamplers.size() << " CLSampler(s)!");
        }
    }
    ...
    std::string CLAbstractionLayer::getCurrentDeviceName()
    {
        return (boost::format("Current CL device: %1% (%2%)") % getDeviceInfo<std::string>(getDeviceId(), CL_DEVICE_NAME) % getDeviceInfo<std::string>(getDeviceId(), CL_DEVICE_VENDOR)).str();
    }

    void CLAbstractionLayer::printCurrentDeviceName()
    {
        LOGGER_IMP(getCurrentDeviceName());
    }
    ...
    void CLAbstractionLayer::createCommandQueue()
    {
        ...
        isProfilingEnabled = framework->getParameterSet()->get<DebugParameterSet>()->getProfilingLevel() >= DebugParameterSet::ProfilingLevel::NORMAL;
    ...
    void CLAbstractionLayer::profileAndReleaseEvent(const std::string& eventCaption, cl_event evt, bool forceProfiling /*= false*/)
    {
        if( (!isProfilingEnabled) || ( (framework->getProfilingLevel() < DebugParameterSet::ProfilingLevel::TRANSFER_TIMING) && !forceProfiling) )
        {
