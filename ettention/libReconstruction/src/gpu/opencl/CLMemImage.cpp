...
//         CL_ASSERT(err);
    ...
    void CLMemImage::transferFrom(const void* source, std::size_t sourceByteWidth)
    {
        if(sourceByteWidth != 0 && sourceByteWidth != byteWidth)
        {
            throw Exception("Custom byte width for CLMemImage transfer functions not supported!");
        }
    ...
    void CLMemImage::transferTo(void* dest, std::size_t destByteWidth)
    {
        if(destByteWidth != 0 && destByteWidth != byteWidth)
        {
            throw Exception("Custom byte width for CLMemImage transfer functions not supported!");
        }
        size_t origin[3] = { 0, 0, 0, };
