...
namespace ettention
{
    ...
    ScannerGeometry* ImageStackDirectoryDatasource::createScannerGeometry(const HyperStackIndex& index, AlgebraicParameterSet* paramSet)
    {
        auto findIt = metaDataNodes.find(index);
        if ( findIt == metaDataNodes.end() )
        {
        ...
        return satRotator.createRotatedScannerGeometry(findIt->second.tiltAngle, paramSet->get<InputParameterSet>()->getXAxisTilt());
    }
    ...
    bool ImageStackDirectoryDatasource::isEveryTiltAngleUnique() const
    {
