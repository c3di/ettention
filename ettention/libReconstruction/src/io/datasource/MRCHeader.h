...
typedef struct
{
    enum DataMode
    {
        IMAGE_BYTES = 0,        //signed or unsigned, depends on values of imodStamp and imodFlags
        IMAGE_SIGNED_16 = 1,
        IMAGE_FLOAT_32 = 2,
        TRANSFORM_COMPLEX_16 = 3,
        TRANSFORM_COMPLEX_32 = 4,
        IMAGE_UNSIGNED_16 = 6,
        IMAGE_BYTES_RGB = 16
    };
    ...
    enum SpecialValues
    {
        IMOD_FLAG_FIELD_USED = 1146047817  //used in imodStamp field.
                                           //if imodStamp field has this value, than imodFlags contains relevant flags, see imodFlagsFieldValues enum
    };

    enum ImodFlagsFieldValue
    {
        IMOD_BYTES_ARE_SIGNED = 1         //if mode == IMAGE_BYTES then the bytes are signed
    };
    ...
    char extraData2[20];    //extra data - not used by IMOD
    ...
    unsigned int nLabel;    //number of labels being used
