...
namespace ettention
{
    ...
    ScannerGeometry* MRCStack::createScannerGeometry(const HyperStackIndex& index, AlgebraicParameterSet* paramSet)
    {
        if(index.getNumberOfDimensions() != 1)
        {
            throw Exception("MRC datasource only supports indices of dimension one");
        }
        return satRotator.createRotatedScannerGeometry(this->tiltAnglesInDegree[index[0]], paramSet->get<InputParameterSet>()->getXAxisTilt());
    }

    ScannerGeometry* MRCStack::createTiltGeometry(float tiltAngleInDegree, AlgebraicParameterSet* paramSet)
    {
        return satRotator.createRotatedScannerGeometry(tiltAngleInDegree, paramSet->get<InputParameterSet>()->getXAxisTilt());
    }
}