...
public:
    ...
    GeometricSetup* getGeometricSetup()
    {
        ...
        return new GeometricSetup(satRotator->getRotatedScannerGeometry(tiltAngleInDegrees));
    }
...
TEST_F(LongObjectCompensationTest, Unit_Long_Object_Compensation) 
{
    ...
    float constantLength = geometricSetup->getScannerGeometry()->getTraversalLengthThroughBoundingBox(volume->getProperties().getVolumeBoundingBox());
