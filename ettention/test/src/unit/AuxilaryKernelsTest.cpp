...
#include "algorithm/imagemanipulation/ThresholdWithHysteresisKernel.h"
...
TEST_F(AuxilaryKernelsTest, AuxKernels_Threshold)
{
    ...
    std::string output_res_10("thresholdTest_pyramid_size13_10_result");
    std::string output_res_05("thresholdTest_pyramid_size13_05_result");
    ...
    auxkernel->run();
    auto result = auxkernel->getOutput();
    this->saveImageAsFile(output_res_10, result);

    auxkernel->setThreshold(0.5f);
    auxkernel->run();
    result = auxkernel->getOutput();
    this->saveImageAsFile(output_res_05, result);
    ...
    ImageComparator::assertImagesAreEqual(makeInputFilePath(input_src),    makeOutputFilePath(input_src));
    ImageComparator::assertImagesAreEqual(makeInputFilePath(input_res_10), makeOutputFilePath(output_res_10));
    ImageComparator::assertImagesAreEqual(makeInputFilePath(input_res_05), makeOutputFilePath(output_res_05));
}

TEST_F(AuxilaryKernelsTest, AuxKernels_ThresholdWithHysteresis)
{
    std::string input_src ("thresholdTest_pyramid_size13_source");
    std::string input_res ("thresholdTest_pyramid_size13_025_075_answer");
    std::string output_res("thresholdTest_pyramid_size13_025_075_result");

    auto *buffer = loadInputImageFileToGPU(input_src);
    auto *auxkernel = new ThresholdWithHysteresisKernel(framework, buffer);

    auxkernel->setThreshold(0.25f, 0.75f);
    auxkernel->run();
    auto result = auxkernel->getOutput();
    this->saveImageAsFile(output_res, result);
