...
{
    auto volume = VolumeDeserializer::load(std::string(TESTDATA_DIR) + "/data/volume/volume_test_32bit.mrc", Voxel::DataType::FLOAT_32);
...
{
    auto volume = VolumeDeserializer::load(std::string(TESTDATA_DIR) + "/data/volume/volume_test_16bit.mrc", Voxel::DataType::FLOAT_32);
    voidCheckPatternInVolume(volume);
    delete volume;
}
...
{
    auto volume = VolumeDeserializer::load(std::string(TESTDATA_DIR) + "/data/volume/volume_test_8bit.mrc", Voxel::DataType::FLOAT_32);
    voidCheckPatternInVolume(volume);
    delete volume;
}
