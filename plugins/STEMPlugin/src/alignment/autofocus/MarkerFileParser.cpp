    ...
    namespace stem
    {
        ...
        void MarkerFileParser::parseMarkerFile(std::istream& iss)
        {
            ...
            while(iss.good())
            {
                ...
                float tiltAngle = dataSource->GetTiltAngle(originalIndex);
                ...
                marker->IntensityZProjection().resize(numberOfProjections);
                for(int zPosition = 0; zPosition < numberOfProjections; zPosition++) 
                {
                    ...
                    marker->IntensityZProjection()[ zPosition ] = intensity;
                }
            }
