...
float getRequiredBlurrRadiusForDistanceFromFocalPlane(float distanceFromFocalPlane, __constant ConvergentBeamsParameter *convergentBeamsParameter)
{
    return distanceFromFocalPlane * convergentBeamsParameter->openingHalfAngleTangens;
}

float lookUpPrefilteredResidualValue(float2 clampedCoordinatesInProjection, int2 projectionSize, __global float* residual, float distanceFromFocalPlane, __constant ConvergentBeamsParameter *convergentBeamsParameter, unsigned int numberOfPrecomputedResiduals)
{
    float radius = getRequiredBlurrRadiusForDistanceFromFocalPlane(distanceFromFocalPlane, convergentBeamsParameter);
...
__kernel void correct(int2 projectionResolution, __global float* projection, __global float* vol, __global float* ray_length_inside_volume, __constant ProjectionGeometry *projectionGeometry, __constant Matrix4x4* transform, __constant VolumeGeometry* volumeGeometry, __constant ConvergentBeamsParameter* convergentBeamsParameter, float lambda, unsigned int numberOfPrecomputedResiduals)
{
    ...
    const float residualValue = lookUpPrefilteredResidualValue(clampedCoordinatesInProjection, projectionResolution, projection, distanceFromFocalPlane, convergentBeamsParameter, numberOfPrecomputedResiduals);
